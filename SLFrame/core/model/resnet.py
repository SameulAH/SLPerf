...
class ResNet110(nn.Module):
    def __init__(self, block=BasicBlock, num_classes=10, norm_layer=None):
        super(ResNet110, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer

        self.inplanes = 16  # Start with 16 filters for CIFAR
        self.conv1 = conv3x3(3, self.inplanes)
        self.bn1 = norm_layer(self.inplanes)
        self.relu = nn.ReLU(inplace=True)

        n = 18  # Number of blocks per layer for ResNet-110
    ...
    def forward(self, x):
        x = self.relu(self.bn1(self.conv1(x)))  # B x 16 x 32 x 32
        x = self.layer1(x)  # B x 16 x 32 x 32
        x = self.layer2(x)  # B x 32 x 16 x 16
        x = self.layer3(x)  # B x 64 x 8 x 8
        x = self.avgpool(x)  # B x 64 x 1 x 1
...
class ResNetClient(nn.Module):
    def __init__(self, block=BasicBlock, norm_layer=None):
        ...
        n = 18  # blocks per layer for ResNet110
...
class ResNetServer(nn.Module):
    def __init__(self, block=BasicBlock, norm_layer=None, num_classes=10):
        super(ResNetServer, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer

        self.inplanes = 16  # Must match client output channels
