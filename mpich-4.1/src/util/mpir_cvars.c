 ...
 *   at:   Fri Jan 27 19:55:05 2023 UTC
 *
 * DO NOT EDIT!!!
 */
...
int MPIR_T_cvar_init(void)
{
    ...
    /* declared in src/include/mpir_request.h */
...
"k_reduce_scatter_allgather - Force reduce scatter allgather algorithm");
    ...
    MPIR_T_CVAR_REGISTER_STATIC(
        ...
        defaultval,
        "COLLECTIVE", /* category */
        "Tree type for tree based allreduce knomial_1 is default as it supports both commutative and non-commutative reduce operations kary      - kary tree type knomial_1 - knomial_1 tree type (tree grows starting from the left of the root) knomial_2 - knomial_2 tree type (tree grows starting from the right of the root)");
    tmp_str = defaultval.str;
    got_rc = 0;
...
"tsp_linear        - Force generic transport based linear algorithm");
    ...
    got_rc += rc;
    if (got_rc && debug) {
        printf("CVAR: MPIR_CVAR_GATHERV_INTER_SSEND_MIN_PROCS = %d\n", MPIR_CVAR_GATHERV_INTER_SSEND_MIN_PROCS);
    }
