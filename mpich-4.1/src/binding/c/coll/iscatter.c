...
int MPI_Iscatter(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf,
                 ...
                 MPI_Request *request)  __attribute__ ((weak, alias("PMPI_Iscatter")));
#endif
...
{
    return internal_Iscatter(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, root, comm, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Iscatter(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf,
                 int recvcount, MPI_Datatype recvtype, int root, MPI_Comm comm,
                 MPI_Request *request)
{
    ...
    QMPI_Iscatter_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Iscatter(context, 0, sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype,
                             root, comm, request);

    fn_ptr = (QMPI_Iscatter_t *) MPIR_QMPI_first_fn_ptrs[MPI_ISCATTER_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_ISCATTER_T], sendbuf, sendcount,
...
int MPI_Iscatter_c(const void *sendbuf, MPI_Count sendcount, MPI_Datatype sendtype, void *recvbuf,
                   ...
                   MPI_Request *request)  __attribute__ ((weak, alias("PMPI_Iscatter_c")));
#endif
/* -- End Profiling Symbol Block */
...
{
    return internal_Iscatter_c(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, root, comm, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Iscatter_c(const void *sendbuf, MPI_Count sendcount, MPI_Datatype sendtype, void *recvbuf,
                   MPI_Count recvcount, MPI_Datatype recvtype, int root, MPI_Comm comm,
...
{
    QMPI_Context context;
    QMPI_Iscatter_c_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Iscatter_c(context, 0, sendbuf, sendcount, sendtype, recvbuf, recvcount,
                               recvtype, root, comm, request);

    fn_ptr = (QMPI_Iscatter_c_t *) MPIR_QMPI_first_fn_ptrs[MPI_ISCATTER_C_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_ISCATTER_C_T], sendbuf, sendcount,
            sendtype, recvbuf, recvcount, recvtype, root, comm, request);
}
#else /* ENABLE_QMPI */
