...
int MPI_Allgather_init(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf,
                       ...
                       MPI_Request *request)  __attribute__ ((weak, alias("PMPI_Allgather_init")));
#endif
...
int QMPI_Allgather_init(QMPI_Context context, int tool_id, const void *sendbuf, int sendcount,
                        ...
                        MPI_Comm comm, MPI_Info info, MPI_Request *request)
{
    return internal_Allgather_init(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm, info, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Allgather_init(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf,
                       int recvcount, MPI_Datatype recvtype, MPI_Comm comm, MPI_Info info,
                       MPI_Request *request)
{
    ...
    QMPI_Allgather_init_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Allgather_init(context, 0, sendbuf, sendcount, sendtype, recvbuf, recvcount,
                                   recvtype, comm, info, request);

    fn_ptr = (QMPI_Allgather_init_t *) MPIR_QMPI_first_fn_ptrs[MPI_ALLGATHER_INIT_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_ALLGATHER_INIT_T], sendbuf, sendcount,
            sendtype, recvbuf, recvcount, recvtype, comm, info, request);
}
#else /* ENABLE_QMPI */
...
int QMPI_Allgather_init_c(QMPI_Context context, int tool_id, const void *sendbuf,
                          ...
                          MPI_Request *request)
{
    return internal_Allgather_init_c(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm, info, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Allgather_init_c(const void *sendbuf, MPI_Count sendcount, MPI_Datatype sendtype,
                         void *recvbuf, MPI_Count recvcount, MPI_Datatype recvtype, MPI_Comm comm,
...
{
    QMPI_Context context;
    QMPI_Allgather_init_c_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Allgather_init_c(context, 0, sendbuf, sendcount, sendtype, recvbuf, recvcount,
                                     recvtype, comm, info, request);

    fn_ptr = (QMPI_Allgather_init_c_t *) MPIR_QMPI_first_fn_ptrs[MPI_ALLGATHER_INIT_C_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_ALLGATHER_INIT_C_T], sendbuf, sendcount,
            sendtype, recvbuf, recvcount, recvtype, comm, info, request);
}
#else /* ENABLE_QMPI */
