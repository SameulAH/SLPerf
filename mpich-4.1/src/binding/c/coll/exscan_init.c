...
int QMPI_Exscan_init(QMPI_Context context, int tool_id, const void *sendbuf, void *recvbuf,
                     ...
                     MPI_Request *request)
{
    return internal_Exscan_init(sendbuf, recvbuf, count, datatype, op, comm, info, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Exscan_init(const void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, MPI_Op op,
                    MPI_Comm comm, MPI_Info info, MPI_Request *request)
{
    ...
    QMPI_Exscan_init_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Exscan_init(context, 0, sendbuf, recvbuf, count, datatype, op, comm, info,
                                request);

    fn_ptr = (QMPI_Exscan_init_t *) MPIR_QMPI_first_fn_ptrs[MPI_EXSCAN_INIT_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_EXSCAN_INIT_T], sendbuf, recvbuf, count,
...
{
    return internal_Exscan_init_c(sendbuf, recvbuf, count, datatype, op, comm, info, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Exscan_init_c(const void *sendbuf, void *recvbuf, MPI_Count count, MPI_Datatype datatype,
                      MPI_Op op, MPI_Comm comm, MPI_Info info, MPI_Request *request)
{
    QMPI_Context context;
    QMPI_Exscan_init_c_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Exscan_init_c(context, 0, sendbuf, recvbuf, count, datatype, op, comm, info,
                                  request);

    fn_ptr = (QMPI_Exscan_init_c_t *) MPIR_QMPI_first_fn_ptrs[MPI_EXSCAN_INIT_C_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_EXSCAN_INIT_C_T], sendbuf, recvbuf,
            count, datatype, op, comm, info, request);
}
#else /* ENABLE_QMPI */
