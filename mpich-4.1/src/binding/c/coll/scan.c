...
int MPI_Scan(const void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, MPI_Op op,
             MPI_Comm comm)  __attribute__ ((weak, alias("PMPI_Scan")));
#endif
...
{
    return internal_Scan(sendbuf, recvbuf, count, datatype, op, comm);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Scan(const void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, MPI_Op op,
             MPI_Comm comm)
...
int MPI_Scan_c(const void *sendbuf, void *recvbuf, MPI_Count count, MPI_Datatype datatype,
               MPI_Op op, MPI_Comm comm)  __attribute__ ((weak, alias("PMPI_Scan_c")));
#endif
/* -- End Profiling Symbol Block */
                           ...
                           MPI_Datatype datatype, MPI_Op op, MPI_Comm comm)
{
    ...
    if (sizeof(MPI_Count) == sizeof(MPI_Aint)) {
        MPIR_Errflag_t errflag = MPIR_ERR_NONE;
        mpi_errno = MPIR_Scan(sendbuf, recvbuf, (MPI_Aint) count, datatype, op, comm_ptr, &errflag);
        if (mpi_errno) {
            goto fn_fail;
        }
    } else {
...
{
    return internal_Scan_c(sendbuf, recvbuf, count, datatype, op, comm);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Scan_c(const void *sendbuf, void *recvbuf, MPI_Count count, MPI_Datatype datatype,
               MPI_Op op, MPI_Comm comm)
{
    QMPI_Context context;
    QMPI_Scan_c_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Scan_c(context, 0, sendbuf, recvbuf, count, datatype, op, comm);

    fn_ptr = (QMPI_Scan_c_t *) MPIR_QMPI_first_fn_ptrs[MPI_SCAN_C_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_SCAN_C_T], sendbuf, recvbuf, count,
            datatype, op, comm);
}
#else /* ENABLE_QMPI */
