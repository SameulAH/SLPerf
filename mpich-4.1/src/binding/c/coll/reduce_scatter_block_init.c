...
{
    return internal_Reduce_scatter_block_init(sendbuf, recvbuf, recvcount, datatype, op, comm, info, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Reduce_scatter_block_init(const void *sendbuf, void *recvbuf, int recvcount,
                                  MPI_Datatype datatype, MPI_Op op, MPI_Comm comm, MPI_Info info,
                                  MPI_Request *request)
{
    ...
    QMPI_Reduce_scatter_block_init_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Reduce_scatter_block_init(context, 0, sendbuf, recvbuf, recvcount, datatype, op,
                                              comm, info, request);

    fn_ptr = (QMPI_Reduce_scatter_block_init_t *) MPIR_QMPI_first_fn_ptrs[MPI_REDUCE_SCATTER_BLOCK_INIT_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_REDUCE_SCATTER_BLOCK_INIT_T], sendbuf,
            recvbuf, recvcount, datatype, op, comm, info, request);
}
#else /* ENABLE_QMPI */
...
{
    return internal_Reduce_scatter_block_init_c(sendbuf, recvbuf, recvcount, datatype, op, comm, info, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Reduce_scatter_block_init_c(const void *sendbuf, void *recvbuf, MPI_Count recvcount,
                                    MPI_Datatype datatype, MPI_Op op, MPI_Comm comm, MPI_Info info,
...
{
    QMPI_Context context;
    QMPI_Reduce_scatter_block_init_c_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Reduce_scatter_block_init_c(context, 0, sendbuf, recvbuf, recvcount, datatype,
                                                op, comm, info, request);

    fn_ptr = (QMPI_Reduce_scatter_block_init_c_t *) MPIR_QMPI_first_fn_ptrs[MPI_REDUCE_SCATTER_BLOCK_INIT_C_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_REDUCE_SCATTER_BLOCK_INIT_C_T], sendbuf,
            recvbuf, recvcount, datatype, op, comm, info, request);
}
#else /* ENABLE_QMPI */
