...
{
    return internal_Win_allocate(size, disp_unit, info, comm, baseptr, win);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Win_allocate(MPI_Aint size, int disp_unit, MPI_Info info, MPI_Comm comm, void *baseptr,
                     MPI_Win *win)
...
int QMPI_Win_allocate_c(QMPI_Context context, int tool_id, MPI_Aint size, MPI_Aint disp_unit,
                        MPI_Info info, MPI_Comm comm, void *baseptr, MPI_Win *win)
{
    return internal_Win_allocate_c(size, disp_unit, info, comm, baseptr, win);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Win_allocate_c(MPI_Aint size, MPI_Aint disp_unit, MPI_Info info, MPI_Comm comm,
                       void *baseptr, MPI_Win *win)
{
    QMPI_Context context;
    QMPI_Win_allocate_c_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Win_allocate_c(context, 0, size, disp_unit, info, comm, baseptr, win);

    fn_ptr = (QMPI_Win_allocate_c_t *) MPIR_QMPI_first_fn_ptrs[MPI_WIN_ALLOCATE_C_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_WIN_ALLOCATE_C_T], size, disp_unit,
