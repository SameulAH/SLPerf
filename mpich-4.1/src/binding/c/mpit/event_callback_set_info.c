...
int QMPI_T_event_callback_set_info(QMPI_Context context, int tool_id,
                                   ...
                                   MPI_T_cb_safety cb_safety, MPI_Info info)
{
    return internal_T_event_callback_set_info(event_registration, cb_safety, info);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_T_event_callback_set_info(MPI_T_event_registration event_registration,
                                  MPI_T_cb_safety cb_safety, MPI_Info info)
{
    ...
    QMPI_T_event_callback_set_info_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_T_event_callback_set_info(context, 0, event_registration, cb_safety, info);

    fn_ptr = (QMPI_T_event_callback_set_info_t *) MPIR_QMPI_first_fn_ptrs[MPI_T_EVENT_CALLBACK_SET_INFO_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_T_EVENT_CALLBACK_SET_INFO_T],
            event_registration, cb_safety, info);
}
#else /* ENABLE_QMPI */
