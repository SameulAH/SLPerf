...
int QMPI_T_event_handle_free(QMPI_Context context, int tool_id,
                             ...
                             MPI_T_event_free_cb_function free_cb_function)
{
    return internal_T_event_handle_free(event_registration, user_data, free_cb_function);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_T_event_handle_free(MPI_T_event_registration event_registration, void *user_data,
                            MPI_T_event_free_cb_function free_cb_function)
{
    ...
    QMPI_T_event_handle_free_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_T_event_handle_free(context, 0, event_registration, user_data,
                                        free_cb_function);

    fn_ptr = (QMPI_T_event_handle_free_t *) MPIR_QMPI_first_fn_ptrs[MPI_T_EVENT_HANDLE_FREE_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_T_EVENT_HANDLE_FREE_T],
            event_registration, user_data, free_cb_function);
}
#else /* ENABLE_QMPI */
