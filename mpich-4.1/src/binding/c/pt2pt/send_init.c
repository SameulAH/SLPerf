...
{
    return internal_Send_init(buf, count, datatype, dest, tag, comm, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Send_init(const void *buf, int count, MPI_Datatype datatype, int dest, int tag,
                  MPI_Comm comm, MPI_Request *request)
{
    ...
    QMPI_Send_init_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Send_init(context, 0, buf, count, datatype, dest, tag, comm, request);

    fn_ptr = (QMPI_Send_init_t *) MPIR_QMPI_first_fn_ptrs[MPI_SEND_INIT_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_SEND_INIT_T], buf, count, datatype,
...
int QMPI_Send_init_c(QMPI_Context context, int tool_id, const void *buf, MPI_Count count,
                     MPI_Datatype datatype, int dest, int tag, MPI_Comm comm, MPI_Request *request)
{
    return internal_Send_init_c(buf, count, datatype, dest, tag, comm, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Send_init_c(const void *buf, MPI_Count count, MPI_Datatype datatype, int dest, int tag,
                    MPI_Comm comm, MPI_Request *request)
{
    QMPI_Context context;
    QMPI_Send_init_c_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Send_init_c(context, 0, buf, count, datatype, dest, tag, comm, request);

    fn_ptr = (QMPI_Send_init_c_t *) MPIR_QMPI_first_fn_ptrs[MPI_SEND_INIT_C_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_SEND_INIT_C_T], buf, count, datatype,
            dest, tag, comm, request);
}
#else /* ENABLE_QMPI */
