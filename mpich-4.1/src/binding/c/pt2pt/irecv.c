...
{
    return internal_Irecv(buf, count, datatype, source, tag, comm, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Irecv(void *buf, int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm,
              MPI_Request *request)
...
{
    return internal_Irecv_c(buf, count, datatype, source, tag, comm, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Irecv_c(void *buf, MPI_Count count, MPI_Datatype datatype, int source, int tag,
                MPI_Comm comm, MPI_Request *request)
{
    QMPI_Context context;
    QMPI_Irecv_c_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Irecv_c(context, 0, buf, count, datatype, source, tag, comm, request);

    fn_ptr = (QMPI_Irecv_c_t *) MPIR_QMPI_first_fn_ptrs[MPI_IRECV_C_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_IRECV_C_T], buf, count, datatype,
