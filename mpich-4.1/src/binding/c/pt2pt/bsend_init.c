...
{
    return internal_Bsend_init(buf, count, datatype, dest, tag, comm, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Bsend_init(const void *buf, int count, MPI_Datatype datatype, int dest, int tag,
                   MPI_Comm comm, MPI_Request *request)
...
int QMPI_Bsend_init_c(QMPI_Context context, int tool_id, const void *buf, MPI_Count count,
                      ...
                      MPI_Request *request)
{
    return internal_Bsend_init_c(buf, count, datatype, dest, tag, comm, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Bsend_init_c(const void *buf, MPI_Count count, MPI_Datatype datatype, int dest, int tag,
                     MPI_Comm comm, MPI_Request *request)
{
    QMPI_Context context;
    QMPI_Bsend_init_c_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Bsend_init_c(context, 0, buf, count, datatype, dest, tag, comm, request);

    fn_ptr = (QMPI_Bsend_init_c_t *) MPIR_QMPI_first_fn_ptrs[MPI_BSEND_INIT_C_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_BSEND_INIT_C_T], buf, count, datatype,
            dest, tag, comm, request);
}
#else /* ENABLE_QMPI */
