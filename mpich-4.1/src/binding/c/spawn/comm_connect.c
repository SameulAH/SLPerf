...
{
    return internal_Comm_connect(port_name, info, root, comm, newcomm);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Comm_connect(const char *port_name, MPI_Info info, int root, MPI_Comm comm,
                     MPI_Comm *newcomm)
{
    ...
    QMPI_Comm_connect_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Comm_connect(context, 0, port_name, info, root, comm, newcomm);

    fn_ptr = (QMPI_Comm_connect_t *) MPIR_QMPI_first_fn_ptrs[MPI_COMM_CONNECT_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_COMM_CONNECT_T], port_name, info, root,
            comm, newcomm);
}
#else /* ENABLE_QMPI */
