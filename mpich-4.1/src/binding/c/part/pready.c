...
int MPI_Pready(int partition, MPI_Request request)  __attribute__ ((weak, alias("PMPI_Pready")));
#endif
...
{
    return internal_Pready(partition, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Pready(int partition, MPI_Request request)
{
    QMPI_Context context;
    QMPI_Pready_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Pready(context, 0, partition, request);

    fn_ptr = (QMPI_Pready_t *) MPIR_QMPI_first_fn_ptrs[MPI_PREADY_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_PREADY_T], partition, request);
}
#else /* ENABLE_QMPI */
