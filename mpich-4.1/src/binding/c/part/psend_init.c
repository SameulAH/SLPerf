...
int QMPI_Psend_init(QMPI_Context context, int tool_id, const void *buf, int partitions,
                    ...
                    MPI_Info info, MPI_Request *request)
{
    return internal_Psend_init(buf, partitions, count, datatype, dest, tag, comm, info, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Psend_init(const void *buf, int partitions, MPI_Count count, MPI_Datatype datatype,
                   int dest, int tag, MPI_Comm comm, MPI_Info info, MPI_Request *request)
{
    ...
    QMPI_Psend_init_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Psend_init(context, 0, buf, partitions, count, datatype, dest, tag, comm, info,
                               request);

    fn_ptr = (QMPI_Psend_init_t *) MPIR_QMPI_first_fn_ptrs[MPI_PSEND_INIT_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_PSEND_INIT_T], buf, partitions, count,
