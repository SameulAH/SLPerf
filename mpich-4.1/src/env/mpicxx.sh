...
if [ "$show_info" = compile ] ; then
    ...
    if [ "$nativelinking" = yes ] ; then
        ...
        if [ "$static_mpi" = no ] ; then
            echo ${final_ldflags} -L$libdir $cxxlibs $PROFILE_PRELIB $PROFILE_FOO ${wrapper_dl_type_flags} -lmpi  $PROFILE_POSTLIB ${final_libs}
        else
            echo ${final_ldflags} -L$libdir $cxxlibs $PROFILE_PRELIB $PROFILE_FOO ${wrapper_dl_type_flags} $libdir/libmpi.a  $PROFILE_POSTLIB ${final_libs}
        fi
    fi
